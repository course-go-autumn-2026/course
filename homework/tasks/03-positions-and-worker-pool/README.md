...
4. **Запрос выборки поездок для опроса** — один SQL, который отдаёт только то,
   что надо опросить ([п. 5.2](#52-выборка-поездок-для-опроса)).
5. **Сканер**: фоновая горутина с тикером, которая на каждом тике забирает эту
   выборку и ставит поездки в очередь ([п. 5.3](#53-сканер)).
6. **Очередь и worker pool**: ограниченная очередь, фиксированное число
   воркеров, дедупликация, отмена по пришедшей координате ([п. 5.4](#54-очередь-и-worker-pool)).
7. **Гейтвей в Push Service** по HTTP, за интерфейсом, с таймаутом ([п. 5.5](#55-гейтвей-в-push-service)).
8. **Метрики** пула и проверок из
   [`docs/observability.md`](../../docs/observability.md).
9. **Тесты на конкурентность**: `-race`, `goleak`, подменяемое время ([п. 7](#7-тесты)).
...
Координаты приходят не по порядку: устройство копит их в офлайне и присылает
пачкой. Поэтому обновление `last_position_at` должно быть устойчиво к
опозданиям — старая координата, доехавшая позже свежей, не должна состарить
поездку. Сравнение делается в самом запросе, а не чтением значения в Go с
последующей записью. На защите про это спросят.
...
каждом тике выполняет запрос из [п. 5.2](#52-выборка-поездок-для-опроса) и ставит полученные поездки в очередь.
...
1. **Выборка идёт без `LIMIT`**, за раз тянем все подходящие поездки. При
   ...
   с последнего ключа. Это задание со звёздочкой, см. [п. 10](#10-критерии-оценки).
...
1. интеграционный на запрос из [п. 5.2](#52-выборка-поездок-для-опроса): свежая и завершённая поездки в выборку не
...
| 3 | Запрос выборки из [п. 5.2](#52-выборка-поездок-для-опроса) отбирает нужные поездки, условие по времени считается в SQL, завершённые и свежие не попадают | 1 |
...
| 7 | Остановка по [п. 5.7](#57-остановка): порядок соблюдён, задачи не теряются, `goleak` и `-race` зелёные | 1 |
| 8 | Метрики [п. 6](#6-что-должно-быть-видно-в-телеметрии) и тесты [п. 7](#7-тесты), включая подменяемое время и сценарии переполнения и отмены | 1 |
...
Убираем упрощение из [п. 5.2](#52-выборка-поездок-для-опроса): сканер перестаёт тянуть все подходящие поездки
разом. Требования:
